  ...
  return (
    ...
    >
      <div className="relative mb-6 h-48 w-full overflow-hidden rounded-[1.15rem] bg-secondary">
        {course.image ? (
          <Image src={course.image} alt={course.title} fill quality={68} sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition-transform duration-500 group-hover:scale-105" />
        ) : (
      ...
      <div>
        ...
        <a href="#contact" className="inline-flex items-center font-medium text-primary transition-colors hover:text-primary/80">
          {course.cta} <ChevronRight size={16} className="ml-1 transition-transform group-hover:translate-x-1" />
        </a>
      </div>
    </div>
  )
...
export function ClassesSection({ isVisible, setSectionRef, coursesExpanded, setCoursesExpanded }: ClassesSectionProps) {
  return (
    <section
      id="services"
      ref={(element) => { setSectionRef("services", element) }}
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-16 px-4 text-left lg:px-0 lg:text-center">
          ...
          <p className="max-w-none text-lg text-pretty text-foreground lg:mx-auto lg:max-w-2xl">Each treatment is customized to your features and skin tone for results that look natural — never overdone.</p>
        </div>
        {courseCategories.map((category, categoryIndex) => {
          ...
          return (
            <div key={category.label} className={`${categoryIndex > 0 ? "mt-12" : ""} ${isCategoryHiddenOnMobile ? "hidden md:block" : "block"}`}>
