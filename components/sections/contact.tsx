                      ...
                      {courseDropdownOpen && (
                        <div className="absolute right-0 left-0 z-[10020] mt-2 overflow-hidden rounded-xl bg-[#0A0A0A] shadow-[0_22px_60px_rgba(0,0,0,0.72)]" style={{ border: "1px solid rgba(212,175,55,0.25)" }} role="listbox">
                          {courseOptions.map((course) => (
                            <button key={course} type="button" className="block min-h-12 w-full border-b border-[rgba(212,175,55,0.15)] bg-[#0A0A0A] px-4 py-3 text-left text-sm font-semibold tracking-wide text-white transition-colors duration-200 last:border-b-0 hover:bg-[rgba(212,175,55,0.14)] hover:text-white focus:bg-[rgba(212,175,55,0.14)] focus:text-white focus:outline-none" onClick={() => { setSelectedCourse(course); setCourseDropdownOpen(false); setFormErrors((previous) => ({ ...previous, corso: false })); validateField("corso", course) }} role="option" aria-selected={selectedCourse === course}>
                              {course}
                            </button>
                          ))}
                        </div>
            ...
            <div className="space-y-8">
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <ContactLink href={brand.facebookHref} icon={<MessageCircle className="mt-1 h-6 w-6 flex-shrink-0 text-primary transition-transform group-hover:scale-110" />} label="Facebook" value="Centro Danza" />
                </div>
              </div>
            </div>
          </div>
