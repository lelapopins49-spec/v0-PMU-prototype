...
export function Navigation({ navLinks, activeSection, isScrolled, mobileMenuOpen, setMobileMenuOpen }: NavigationProps) {
  ...
  return (
    <>
      {announcementVisible && (
        <div className={`fixed top-0 left-0 right-0 z-[60] border-b border-[#D4AF37]/30 transition-all duration-400 ${announcementFading ? "-translate-y-full opacity-0" : "translate-y-0 opacity-100"}`} style={{ background: "linear-gradient(90deg, #1A0A0A 0%, #2A0E1A 50%, #1A0A0A 100%)" }}>
          <div className="mx-auto flex max-w-7xl items-center justify-center gap-2 px-4 py-2 sm:px-6 lg:px-8">
            ...
            <span className="text-center text-xs font-medium tracking-wide text-white/80 sm:text-sm">Your first consultation is free &mdash; book today with no obligation</span>
      ...
      <nav className={`fixed left-0 right-0 transition-[background-color,border-color,box-shadow] duration-300 ${mobileMenuOpen ? "z-[9500]" : "z-50"} ${announcementVisible && !announcementFading ? "top-[36px]" : "top-0"}`} style={{ backgroundColor: isScrolled ? "rgba(10,10,10,0.96)" : "rgba(10,10,10,0.82)", borderBottom: isScrolled ? "1px solid rgba(192,21,42,0.2)" : "1px solid rgba(192,21,42,0.1)", boxShadow: isScrolled ? "0 8px 24px rgba(10,10,10,0.42)" : "none" }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-28 items-center justify-between gap-5 py-5 sm:h-32 lg:h-32">
            <a href="#" className="relative z-[60] flex max-w-[260px] flex-col items-start leading-none sm:max-w-[320px]">
              ...
              <span className="mt-2 text-[9px] font-semibold tracking-[0.2em] text-[#F5F5F5]/70 uppercase sm:text-[10px]">{brand.tagline}</span>
            </a>
            <div className="hidden items-center gap-8 lg:flex">
              ...
              <a href="#contact" className="rounded-full border-2 border-white px-6 py-2 font-semibold text-white transition-colors hover:bg-white hover:text-[#0F0F0F]">Book Now</a>
            </div>
        ...
        <div className={`fixed inset-0 z-[9500] flex min-h-[100dvh] w-screen items-center justify-center bg-[rgba(10,10,10,0.95)] px-4 py-20 transition-opacity duration-300 lg:hidden ${mobileMenuOpen ? "opacity-100" : "pointer-events-none opacity-0"}`} onClick={() => setMobileMenuOpen(false)} onTouchStart={(event) => { touchStartXRef.current = event.touches[0].clientX }} onTouchEnd={(event) => { if (touchStartXRef.current !== null) { const delta = event.changedTouches[0].clientX - touchStartXRef.current; if (delta > 60) setMobileMenuOpen(false); touchStartXRef.current = null } }}>
          <div className={`relative w-[calc(100%-32px)] max-w-[430px] overflow-hidden rounded-[1.35rem] border border-white/20 bg-[#0F0F0F] p-6 shadow-[0_24px_70px_rgba(10,10,10,0.95)] transition-all duration-300 sm:p-7 ${mobileMenuOpen ? "translate-y-0 scale-100 opacity-100" : "translate-y-4 scale-95 opacity-0"}`} onClick={(event) => event.stopPropagation()}>
            ...
            <div className="relative mb-6 border-b border-white/15 pb-6 text-center">
              ...
              <p className="mt-2 text-[9px] font-semibold tracking-[0.2em] text-[#F5F5F5]/70 uppercase">{brand.tagline}</p>
            </div>
            <div className="relative flex flex-col gap-2.5">
              ...
              <a href="#contact" className="rounded-full border-2 border-white px-6 py-2 font-semibold text-white transition-all hover:bg-white hover:text-[#0F0F0F]" onClick={() => setMobileMenuOpen(false)}>Book Now</a>
            </div>
          </div>
        </div>
      </nav>
